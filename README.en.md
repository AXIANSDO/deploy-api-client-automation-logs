...
The automation registration step persists the approved automation locally so each execution can reference the correct `automation_id`.

## Why does the customer need to register an automation locally?

This is the part that usually creates confusion, so the rule is explicit:
...
| API | Purpose |
| --- | --- |
| AXIANS central API (`api-axians`) | Stores the approved automation centrally, issues the token, and receives execution logs from all customers |
| Customer local API (`api-client`) | Stores a local copy of the approved automation and receives execution logs from local tools |

The local registration step exists because the customer-side API stores automation metadata locally and links each execution to an existing local automation record.

That means:

- the customer registers the approved automation locally once
- after that, the customer only sends execution logs
- the same `automation_id` is reused end to end

This local registration step does **not**:

- create a new automation in the AXIANS central platform
- replace the approval workflow managed by AXIANS
- require the customer to invent a new identifier

It simply tells the local API:

"This approved AXIANS automation exists in this customer environment, and future execution logs using this `automation_id` belong to it."
...
## One-time setup vs recurring usage

There are two separate actions:

1. One-time local setup
   Register the approved automation in `POST /automations`.

2. Recurring runtime usage
   Send one execution record after each automation run to `POST /executions`.

Most customer tools only need to perform step 1 once, then keep using step 2 forever.

## Important contract detail

When a customer tool sends an execution to the local API, the `automation_id` is sent inside the JSON body.

The route is:

- `POST /executions`

The JSON body sent by the customer tool contains execution data such as:
...
The local API then injects the configured `CLIENT_ID` and forwards the execution to AXIANS using the `automation_id` already present in the payload.
...
- central approval in AXIANS: already done
- local registration in the customer environment: required once before the first execution log
...
Do not include `CLIENT_ID` or `AUTOMATION_TOKEN` in the JSON body sent by the automation tool.
