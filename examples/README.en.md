...
This product is focused on execution logging and execution time tracking. The registration step exists only to persist the approved automation locally before the first execution is sent.

## Why are there examples that create or register automations?

Because the local API needs a one-time local registration step before it can accept execution logs.

This does **not** mean the customer is creating a new automation in AXIANS.

What really happens is:

1. AXIANS approves the automation centrally and provides the official `automation_id`
2. the customer registers that approved automation once in the local API
3. the customer keeps sending execution logs using the same `automation_id`

So the examples that call `POST /automations` are showing the **local registration** step, not a central approval or central automation creation step.
...
- body: execution fields plus `automation_id`

Do not send `CLIENT_ID` or `AUTOMATION_TOKEN` inside the JSON body of the local execution request.
...
- `POST /executions` returns `201` when the log is stored and forwarded immediately
- `POST /executions` returns `202` when the log is queued locally for retry

Treat both `201` and `202` as successful execution submissions.

The `POST /automations` step is a local registration step. It exists because the customer-side API stores automations locally and expects `automation_id` in the execution payload.
